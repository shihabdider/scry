...
test('ensureFavoritesModeReady stores storage failures under favorites without changing ready public caches', async () => {
  ...
  app.modeCache = { recent: recentState }
  app.index = recentState.index
  ...
  assert.equal(app.modeCache.recent, recentState)
  assert.equal(recentState.status, 'ready')
})

test('exitFavoritesModeToPreviousPublicMode restores previous recent mode', async () => {
  const app = panelApp(favoritesChrome([]).chromeApi)
  app.previousPublicSearchMode = 'recent'
  let switchedTo = null
  ...
  assert.equal(switchedTo, 'recent')
})
...
test('exitFavoritesModeToPreviousPublicMode defaults an invalid previous mode to recent', async () => {
  const app = panelApp(favoritesChrome([]).chromeApi)
...
test('enterFavoritesMode from recent clears the command, remembers recent, loads favorites, and shows all favorites', async () => {
  ...
  assert.equal(app.previousPublicSearchMode, 'recent')
...
test('handleSearchInputTab cycles public recent forward to closed', async () => {
  const app = panelApp(favoritesChrome([]).chromeApi)
  app.searchMode = 'recent'
  let switchedTo = null
  app.switchSearchMode = async (mode) => {
...
test('handleSearchInputTab cycles public closed forward to deep', async () => {
  const app = panelApp(favoritesChrome([]).chromeApi)
  app.searchMode = 'closed'
  let switchedTo = null
  app.switchSearchMode = async (mode) => {
  ...
  assert.equal(switchedTo, 'deep')
  assert.equal(app.searchMode, 'deep')
})

test('handleSearchInputTab cycles public recent backward to deep', async () => {
  const app = panelApp(favoritesChrome([]).chromeApi)
  app.searchMode = 'recent'
  let switchedTo = null
  app.switchSearchMode = async (mode) => {
